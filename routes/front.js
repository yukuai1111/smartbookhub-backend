...
router.get('/article-recommend-detail', async (req, res) => {
    ...
    const hasRead = article.userIds.includes(userId)
    // console.log(hasRead)
    if (hasRead) {
        //已经阅读过了
        res.json({ code: '200', msg: "读过文章，获取文章详情成功", data: article, success: true })
        return
    }
    else {
        //还没阅读过
        ...
        //更新数据库的阅读量,并加入用户id到数组Ids
        await articleCollection.updateOne({ id }, { $set: { readCount: String(num) }, $push: { userIds: userId } })  //updateOne({选出的文档}，{更新的内容})
        //手动更新页面阅读量+1
