...
//新增文章接口
router.post('/article-add', upload.single('cover'), async (req, res) => {
    // console.log(req.body)   //文字
    // console.log(req.file)  //图片
...
//更新文章接口
router.put('/article-update', upload.single('cover'), async (req, res) => {
    // console.log('更新',req.body)   //获取文字
    console.log('更新',req.file||'没有图片')   //获取图片
    ...
    if (req.file) {
        ...
        await articleCollection.updateOne({id},{$set:{cover:req.file.filename}})  //更新图片路径
        console.log('新图片路径:', req.file.filename)
    } else {
        // console.log('没有图片')
        //如果有旧图片，就删除
        if (oldCover) {
